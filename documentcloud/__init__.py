...
from .toolbox import (CredentialsFailedError, DoesNotExistError,
                      DuplicateObjectError, credentials_required, retry)
...
class BaseDocumentCloudClient(object):
        ...
        try:
            ...
            if getattr(e, "code", None) == 404:
                raise DoesNotExistError(
                    "The resource you've requested does not exist or is "
                    "unavailable without the proper credentials."
                )
            elif getattr(e, "code", None) == 401:
                raise CredentialsFailedError(
                    "The resource you've requested requires proper credentials."
                )
            else:
                raise e
    ...
    def put(self, method, params):
        ...
        # Make the request
        self._make_request(self.BASE_URI + method, params.encode("utf-8"))
    ...
    def fetch(self, method, params=None):
        ...
        content = self._make_request(self.BASE_URI + method, params)
...
class DocumentClient(BaseDocumentCloudClient):
    ...
    def _get_search_page(self, query, page, per_page=1000, mentions=3, data=False):
        ...
        params = {"q": query, "page": page, "per_page": per_page, "mentions": mentions}
        if data:
            params["data"] = "true"
    ...
    def search(self, query, page=None, per_page=1000, mentions=3, data=False):
        ...
        if page:
            document_list = self._get_search_page(
                query, page=page, per_page=per_page, mentions=mentions, data=data
            )
    ...
    def upload(
        ...
        if size >= 399999999:
            raise ValueError(
                "The pdf you have submitted is over the DocumentCloud API's 400MB "
        ...
        response = self._make_request(
            self.BASE_URI + "upload.json", params, opener=opener
        )
    ...
    def delete(self, id):
        ...
        Deletes a Document.
        """
        self.fetch("documents/%s.json" % id.split("-")[0], {"_method": "delete"})
...
class ProjectClient(BaseDocumentCloudClient):
    ...
    def get(self, id=None, title=None):
        ...
        if id and title:
            ...
            raise ValueError(
                "You must provide an id or a title to make a request."
            )
        # Pull the hits
        if id:
        ...
        if len(hit_list) > 1:
            raise DuplicateObjectError(
                "There is more than one project that matches your request."
            )
    ...
    def delete(self, id):
        ...
        Deletes a Project.
        """
        self.fetch("projects/%s.json" % id, {"_method": "delete"})
    ...
    def _get_url(self, url):
        if self.access == "public":
            req = urllib.request.Request(
                url, headers={"User-Agent": "python-documentcloud"}
            )
...
class DocumentSet(list):
    ...
    def append(self, obj):
        ...
        if not isinstance(obj, Document):
            raise TypeError(
                "Only Document objects can be added to the document_list"
            )
        # Check if the object is already in the list
        if obj.id in [i.id for i in list(self.__iter__())]:
            raise DuplicateObjectError(
                "This object already exists in the document_list"
            )
...
def is_valid_data_keyword(keyword):
    ...
    if keyword in RESERVED_KEYWORDS:
        raise ValueError(
            "The key %s is reserved by DocumentCloud. You can't use it in 'data'"
            % keyword
        )
    else:
        return True
